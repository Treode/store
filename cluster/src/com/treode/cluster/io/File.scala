...
class File (file: AsynchronousFileChannel) {
  ...
  private class FileFiller0 (pos: Long, input: Input, length: Int, cb: Callback [Unit])
  ...
  def fill (pos: Long, input: Input, length: Int, cb: Callback [Unit]): Unit =
    Callback.guard (cb) {
      if (length <= input.limit - input.position) {
        ...
        new FileFiller0 (pos, input, length, cb) .fill()
      }}

  private class FileFlusher0 (output: Output, pos: Long, cb: Callback [Unit])
  extends Callback [Int] {
  ...
  def flush (output: Output, pos: Long, cb: Callback [Unit]): Unit =
    Callback.guard (cb) (new FileFlusher0 (output, pos, cb) .flush())
