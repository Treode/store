...
class AtomicSpec extends FreeSpec with StoreBehaviors with AsyncChecks {
  ...
  def check (mf: Double) (implicit kit: StoreTestKit) {
    ...
    val cb1 = h1.write (xid1, TxClock.MinValue, Create (t, k, 1)) .capture()
    val cb2 = h2.write (xid2, TxClock.MinValue, Create (t, k, 2)) .capture()
  ...
  "The atomic implementation should" - {
    ...
    "rebalance" in {
      ...
      val ts = h1.write (xid, TxClock.MinValue, Create (t, k, 1)) .pass
