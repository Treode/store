...
import com.treode.store.{Bytes, Cardinals, Cell, Fruits, TimedTestTools}
  ...
  "The clean filter should" - {
    ...
    def test (item: (Seq [Cell], Seq [Cell])) {
      ...
      s"handle ${testStringOf (in)}" in {
        ...
        assertResult (out) (TimedFilters.clean (in.iterator.async, 7) .toSeq)
      }}
