    ...
    store.read (rt, ops:_*) .map { vs =>
      ...
      v.value match {
        case Some (value) if ct < v.time =>
          JsonContent ~> Date (httpDate.print(rt.toDateTime)) ~> LastModified (httpDate.print (v.time.toDateTime)) ~> ReadTxClock (rt.toString) ~> ValueTxClock (v.time.toString) ~> Vary ("Request-TxClock") ~> ResponseString (value.toJsonText)
        case Some (_) =>
          NotModified
        case None =>
          NotFound
