...
class MovieResourceSpec extends FreeSpec with SpecTools {
  ...
  "When the database is empty" - {
    ...
    "PUT /movie/1 should respond Ok with an conditionTxClock" in
      served { (port, store) => implicit movies =>
        val r1 = addStarWars (port)
        val t1 = r1.conditionTxClock
    ...
    "POST /movie should respond Ok with an conditionTxClock" in
      served { (port, store) => implicit movies =>
        ...
        given
          .port (port)
        .expect
          .conditionTxClock (r1.conditionTxClock)
          .body (matchesJson (s"""{
              "id": "$id",
              "title": "Star Wars",
              "released" :null,
  ...
  "When the database has a movie" - {
    ...
    "GET /movie/1 with If-Modified-Since:(r1.conditionTxClock) should respond Not Modified" in
      served { (port, store) => implicit movies =>
        val r1 = addStarWars (port)
        given
          .port (port)
          .header ("If-Modified-Since", r1.conditionTxClock.toString)
        .expect
          .statusCode (304)
        .when
          .get ("/movie/1")
    ...
    "GET /movie/1 with Last-Modification-Before:(r1.conditionTxClock-1) should respond Not Found" in
      served { (port, store) => implicit movies =>
        val r1 = addStarWars (port)
        given
          .port (port)
          .header ("Last-Modification-Before", (r1.conditionTxClock-1).toString)
        .expect
          .statusCode (404)
        .when
          .get ("/movie/1")
    ...
    "GET /movie/1 with Last-Modification-Before:(r1.conditionTxClock) should respond Ok" in
      served { (port, store) => implicit movies =>
        val r1 = addStarWars (port)
        given
          .port (port)
          .header ("Last-Modification-Before", (r1.conditionTxClock).toString)
        .expect
          .conditionTxClock (r1.conditionTxClock)
          .body (matchesJson (starWars))
        .when
    ...
    "PUT /movie/1 with should respond Ok with an conditionTxClock" in
      served { (port, store) => implicit movies =>
        ...
        val (t1, t2) = (r1.conditionTxClock, r2.conditionTxClock)
        store.expectCells (PM.MovieTable) (
            ("1", t2, PO.aNewHope),
            ("1", t1, PO.starWars))
        store.expectCells (PM.CastTable) (
    ...
    "PUT /movie/1 with a If-Unmodified-Since:1 should respond Ok with an conditionTxClock" in
      served { (port, store) => implicit movies =>

        val r1 = addStarWars (port)
        val r2 = given
          .port (port)
          .header ("If-Unmodified-Since", r1.conditionTxClock.toString)
          .body (aNewHope)
        .when
          .put ("/movie/1")
