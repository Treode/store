...
class TablesHandlerSpec extends FlatSpec with SpecTools {

  def handler (controller: StoreController) =
    new TablesHandler (controller)

  "The TablesHandler" should "handle GET" in
    served { case (port, controller) =>
      (controller.tables _) .expects() .returning (supply (Seq.empty))
      given
        .port (port)
      .expect
        .statusCode (200)
